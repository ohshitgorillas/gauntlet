...
9. Both the `scrivener` and the main agent work concurrently in different branches, the latter on implementation: `mcp__plugin_gauntlet_pair__impl_checkout` cuts the implementation tree as soon as the spec tree is cut, alongside the `scrivener`, the red run and the `juror`, not after them.
10. The tests run red in the `scrivener`'s tree, and green in the implementation branch. The spec tree that produced the red run never receives the implementation, so that run stays clean of it.
