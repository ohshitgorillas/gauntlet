...
That folder is yours alone. `.claude/hooks/plans-lane.py` denies every other agent, main agent included, every write under `gauntlet/plans/approved/`, so file's existence is only proof a later stage has that plan it reads passed this gate. Write nothing there you did not pass. Rules in `docs/plans.md`.
