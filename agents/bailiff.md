...
**Rules 4, 6, 13 and 14 belong to this job among the rounds that judge a block, and only this one.** A spec block carries no test code, so stage 1 cannot see them; a sweep by `auditor` judges no block — it read tests already in tree, under no slug — so it sit outside what this divide, and every merged block's four rules stay yours; you read `git diff <base> HEAD -- <tests dir>/` from the `diff:` section of `<gauntlet dir>/merge/<slug>.<N>.txt`, and the test files themselves from `<tests dir>/`. Four violations, one row each: a fake speaking over our own code rather than the wire (rule 4, `docs/testing.md:17`), a test name that does not state a behavior (rule 6, `docs/testing.md:21`), a fake deriving its reply by the algorithm the code uses (rule 13, `docs/testing.md:61`), an `assert` outside a `test_*` function (rule 14, `docs/testing.md:63`). One row per violation, no cap, and not a note, since the note slot carries what could not be evaluated and these were. A row forces `ANOTHER PASS`, and its repair is the second of the two below, which is the route that produces the re-approved line `CLAUDE.md`:64 requires of any change to a writer's test. Every other rule stays in stage 1; the block is closed here.
