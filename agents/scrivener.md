...
**`motion: rehome` has you move a test rather than write one.** Its shape is in `docs/testing.md` "Rehome motions". Remove the `strike` target from its file exactly as under `motion: strike`, then write the test its `as:` field names, in the file that field names, carrying the line's quoted `assertion:` text byte-identical. Where the `as:` field names the target itself, nothing is removed: you rewrite that test in place — its fixtures, its arrangement, its imports, whatever the `moved:` fact forces — and carry the quoted assertion through untouched. Byte-identical is the whole motion: a reflow, a renamed local, a changed number or a tightened bound in that assertion is a rewrite, and a rewrite is `motion: amend`'s, not yours. The rest of the test — its fixtures, its arrangement, the import lines the new file needs — you write as the assertion requires, because the target's own file may not be readable from the new one. A whole-file target is malformed under this motion, like an amend's: report it and stop. A target whose body does not hold the quoted assertion is a finding you report and stop on, and so is an in-place line whose body you would leave byte-identical: it records nothing, and the merge check reads it as `UNSATISFIED`. Where the move leaves a file holding no test at all, delete the file.
...
**`motion: strike`, `motion: amend` and `motion: rehome` produce no red run at all.** Removing a test makes the suite greener, an amend's replacement pins behavior the tree already has, and a rehome moves an assertion the tree already satisfies, so it is green on its first run. For those three you report the diff you made — one line per spec line, the target removed and the `as:` name written — and the run that judges it is `${CLAUDE_PLUGIN_ROOT}/scripts/strike-diff.py`, mechanical, at merge.
