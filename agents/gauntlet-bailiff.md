...
**`motion: strike` and `motion: amend` do not reach this job at all.** Neither have implementation phase, so no window exist for test to soften while main agent code against it. `${CLAUDE_PLUGIN_ROOT}/scripts/strike-diff.py`, run by `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh merge`, check those two motions instead: it compare landed `<tests dir>/` diff against committed block by name and by quoted `assertion:` text. Brief for either motion arriving here = contempt format, one line `shape: <what arrived>`.
