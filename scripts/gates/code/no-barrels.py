...
"""Gate: a split moves code, it does not leave a shell behind pointing at where the code went.
    ...
    scripts/gates/code/no-barrels.py [<path>...]
    scripts/gates/code/no-barrels.py --self-test
...
which is exactly what `file-length.py` beside it does, so the two are a pair.
...
`__init__.py` is skipped outright: re-exporting a package's surface is what it is
for, and the shape only lies in a file pretending to be a module of its own. A
module that defines constants defines something, so it is not this — only
`__all__` is discounted, being the re-export's own manifest. A hook or script
whose whole body sits under `if __name__ == "__main__"` defines nothing either,
and is the same shape: the logic belongs in a `main()` this tree can import and
self-test, the way every other entry point here is written.
...
directly checkable, so this checks the two syntactic shapes it comes in. Both
rules reach every path handed to the gate; there is no thin-adapter layer in this
kit for which a one-line pass-through would be the correct shape.

A forwarder that has earned its place — reaching a collaborator that callers have
no other route to is a real job — goes in FORWARDER_EXEMPT with a reason a human
can weigh. A module that legitimately defines nothing goes in MODULE_EXEMPT the
same way. Both lists are audited against the filesystem rather than against the
paths on argv, so a run handed one file still audits the whole table. An entry
excusing nothing fails, so an exemption cannot outlive the thing it excused.
...
argument by keyword (`self.other.f(data, scope=scope)`), and one whose chain is
rooted in a call rather than a name (`self.require_http().restore(...)`). Those
are left to review. A gate that implied it caught every barrel would be worse
...
#: The extensions this gate reads when it picks the file set itself.
TRACKED = ("*.py",)

#: The directories the picked set is kept to — the hooks and scripts this kit ships.
ROOTS = ("hooks/", "scripts/")

#: Module to the reason it defines nothing on purpose. Empty is the healthy
#: state: a module that runs on import still assigns something, one that runs on
#: invocation defines a `main`, and one that re-exports a package's surface is
#: spelled `__init__.py`, which is skipped.
...
def passes_through(func: ast.FunctionDef | ast.AsyncFunctionDef, call: ast.Call) -> bool:
    ...
    `def f(self, x)` forwarding as `self._other.f(x)` still matches: `self` is
    spent on the chain rather than passed.
    """
...
if __name__ == "__main__":
    if "--self-test" in sys.argv:
        from no_barrels_selftest import self_test
