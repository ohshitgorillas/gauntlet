...
"""Gate: no function nests blocks deeper than four levels.
    ...
    scripts/gates/code/nesting.py [<path>...]
    scripts/gates/code/nesting.py --self-test
...
def _block_bodies(node: ast.stmt) -> list[list[ast.stmt]]:
    """Return every statement list a block-opening node hands its body to."""
...
def _block_depth(node: ast.stmt, depth: int, prefix: str, found: _Found) -> int:
    ...
    inner = depth + 1
    return max(_walk(body, inner, prefix, found) for body in _block_bodies(node))
...
if __name__ == "__main__":
    if "--self-test" in sys.argv:
        from nesting_selftest import self_test
