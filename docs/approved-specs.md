...
`gauntlet/specs/approved/` holds the spec blocks that passed adversarial review. It is the boundary between "someone wants this behavior" and "this behavior is the contract", and the blind `gauntlet-scrivener` works from nothing else.
...
- **Drafts sit under the stage, not in the lane.** `gauntlet/specs/` is the stage; the lane is `gauntlet/specs/approved/` inside it, and `gauntlet/specs/drafts/<slug>.txt` is the main agent's own, gitignored, and open to every hand. Nothing about drafting is restricted; the lane governs only the folder a block reaches after review.
...
Reverting is the exception the shell keeps: `git restore --source <rev> -- gauntlet/specs/approved/<file>` passes the hook, because it copies a commit rather than typing a spec.
