...
`check` converges the pair in five steps and lands nothing: the lane check, which holds the spec tree to `<tests dir>/` and the implementation tree out of it; a commit in each tree; a rebase of both branches onto the target branch where it moved under them; the combine, which merges `impl/<slug>` into the spec tree under the subject `merge: <slug>`, because that commit is the combined tree's `HEAD` when the gate reads it; and the gate, run in that combined tree, whose verdict and the three tips it ran over are written to the next numbered reading.
