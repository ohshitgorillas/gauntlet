...
Under `GAUNTLET=off` it is not true. The switch silences `lanes.py`, `no-impl-reads.py`, `bwrap-wrap.py`, `blind-write.py`, `lane-audit.py`, `kit-probe.py` and the `Stop` gate, so a `arbiter` or a `scrivener` spawned in a bypassed session can read the implementation, and nothing denies it. Blindness is the property the whole chain rests on, so a spec block or a test produced in such a session is worth what an unblind agent's work is worth, and it lands in a tracked file that looks like any other. A session with the gauntlet off should not run the chain.
...
8. After the `pair` tool `mcp__plugin_gauntlet_pair__check` gates the pair and `mcp__plugin_gauntlet_pair__merge` lands it, a `bailiff` reads the `TEST CHECK` brief that call returned on stdout, and the `<gauntlet dir>/merge/<slug>.<N>.txt` that brief names, against the committed block, and returns `PIN`, `SOFT`, `MISSING` or `EXTRA` per behavior line, blind. It is the only round that holds test code, so rules 3, 4, 6, 13 and 14 are checked there and nowhere else.
...
`review` is the reviewers' one path into their own lane. The reviewers' row of `lanes.py` denies a reviewer every read of `<gauntlet dir>/reviews/`, so the reviewer cannot count the rounds it is continuing; the main agent calls the `pair` tool `mcp__plugin_gauntlet_pair__review`, or `mcp__plugin_gauntlet_pair__review_plan` for a plan round, before each round that will carry verdicts and hands the returned path to the reviewer verbatim in its brief. A round that writes nothing consumes no `<N>`, because the count is of what is on disk.

The evidence sits in the file the `merge output:` line names, not beneath that header: `<gauntlet dir>/merge/<slug>.<N>.txt` opens with four lines — `spec/<slug>: <sha>`, `impl/<slug>: <sha>` or `-` where no implementation tree was cut, `<target branch>: <sha>`, and `gate: PASS` or `gate: FAIL` — and then carries the changed test file names under `test files:`, `git diff <base> HEAD -- <tests dir>/` under `diff:`, and the saved red log under `red output:`, in that order and under those three headings. The section is empty where `<gauntlet dir>/red/<slug>.txt` is absent.

`open` refuses on mismatch because the spec file is editable after the reviewer passed it, and the round file is not: the comparison is what makes the approved block the reviewed block rather than the latest one. `red` removes the whole-file strike targets, which the lane hook denies every agent, and leaves single-test targets to the writer's `Edit`. `merge` routes on the structure line because the three tests-only shapes have no implementation phase, so the blind post-merge reviewer round has no window to watch and the mechanical check takes it.

`check` converges the pair in five steps and lands nothing: the lane check, which holds the spec tree to `<tests dir>/` and the implementation tree out of it; a commit in each tree; a rebase of both branches onto the target branch where it moved under them; the combine, which merges `impl/<slug>` into the spec tree under the subject `merge: <slug>`, because that commit is the combined tree's `HEAD` when the gate reads it; and the gate, run in that combined tree, whose verdict and the three tips it ran over are written to the next numbered reading. The gate's combined stdout and stderr go to `<gauntlet dir>/gate/<slug>.<N>.txt`, whose `<N>` is that reading's, and stderr carries one `gate log: <path>` line and the log's last 40 lines rather than the whole output, so a gate that prints without bound costs the caller forty lines.

`merge` runs the same first four steps, and its fifth is the reading rather than a gate of its own: the newest reading on disk has to say `gate: PASS` and carry the three tips the verb has in front of it after the combine, or the pair is `UNCHECKED` and nothing lands. Its sixth is the land, a fast-forward of the target branch onto the spec branch, after which both trees and both branches are removed. An implementation tree that was never cut is skipped rather than fatal in either verb, which is the ordinary shape of the three tests-only kinds. Steps three onward hold `flock` on `.claude/worktrees/.pair.lock`, so two sessions converging at once queue instead of racing the tip, and every land is `--ff-only`.

The land writes over the primary checkout, so an owner edit there rides along with a pair in one of two ways: commit it in the primary checkout before `open`, so both trees are cut from it, or copy it into the implementation tree, so it lands as part of the pair. A file in the checkout that is byte for byte the file landing on it gives way: an untracked copy is removed, and a tracked file whose edit is unstaged and whose index entry equals HEAD is restored to HEAD, and either is written back if the fast-forward still refuses. Any other local change over a landing file refuses the land, stays as it is, and is named in the error, which quotes git's own lines.

A red gate stops `check` at its fifth step: nothing lands, both trees stand exactly as they are, and stdout carries the `FAIL` reading rather than a brief — the guidance goes to stderr with the failure and names the gate log, because a brief on stdout is the brief of a merged block. A failing test there means the block and the code disagree, and the way out is the implementation tree or a re-approved block, never an edited test. A reading is kept rather than overwritten, so the red one that sent the work back is still on disk beside the green one that followed it.
...
The two runners are `pytest_command` and `node_command` of that same file, read through the same reader and split into arguments the same way. They default to `.venv/bin/pytest` and `node --test`, and they are what `pair.sh red` runs: a project that has to deselect a marker or import a loader names the whole invocation once there instead of editing the script by hand. The test path and the flags each script adds come after the configured words, and a configured word carrying a slash is a path in the checkout while a bare word is on `PATH`. A wider runner widens nothing a blind agent may type: the `scrivener` and the `bailiff` hold no `Bash`, so the invocation is configuration and never agent input.

What they hold instead is the `blind` MCP server, `scripts/mcp/blind_server.py`, which the manifest wires under `mcpServers` and both definitions admit as `mcp__plugin_gauntlet_blind__*`. It serves three tools, and each runs `scripts/blind.sh` as an argv list with no shell between them: `test` takes a `path`, `status` a `slug`, and `show` a `commit` and a `slug`. Every argument is a typed field checked before anything runs — a test path must lie under `<tests dir>/`, bare or through `.claude/worktrees/<slug>-spec/`, with no `..` part; a slug is lowercase words and hyphens; a commit is a hash, `HEAD` or `HEAD~N` — and a field that fails its type, or one the tool does not declare, comes back as a tool error. `test` returns the narrowed report: one `PASSED`, `FAILED` or `ERROR` line per test id, and under a collection or import error its frames under `<tests dir>/` and its exception line. A frame inside the implementation, a traceback's source line, an assertion's introspection and the lint gates' output never reach the caller, so the run a blind writer checks its own file with shows it no line of the code. The server speaks JSON-RPC through `scripts/mcp/rpc.py`, standard library only, and `python3 scripts/mcp/blind_server.py --self-test` checks it.

The `scrivener` holds one tool more, from a server of its own. `blind-write`, `scripts/mcp/blind_write_server.py`, is wired beside `blind` under `mcpServers` and serves `format`, which takes a `path` checked exactly as `test`'s is and runs `blind.sh format` over that one file: the fix tools of its extension rewrite it under `bwrap`, with the lane bound writable and nothing else. It returns `clean`, or `not clean` when a fault remains that no fix tool repairs, and never the fix tools' output. It is a server apart because it writes, and a server's tools reach every agent that admits its wildcard, which for `blind` includes the `bailiff`. A server the manifest wires is also in front of every agent in the session, so `hooks/blind-write.py`, wired at `PreToolUse` on `mcp__plugin_gauntlet_blind-write__.*`, denies every caller whose `agent_type` is not `scrivener`, the main agent included. `python3 scripts/mcp/blind_write_server.py --self-test` and `python3 hooks/blind-write.py --self-test` check the two.

The main agent runs `pair.sh` through a third server. `pair`, `scripts/mcp/pair_server.py`, is wired beside `blind` and `blind-write` under `mcpServers` and serves one tool per verb in the table above, named `mcp__plugin_gauntlet_pair__<verb>`: `open`, `respec`, `red`, `check`, `merge`, `abort`, `close`, `list`, `review` and `restore`, and `review_plan`, `impl_checkout` and `impl_merge` for the three two-word verbs. Every argument is a typed field, held to the shapes the `blind` server already checks — `slug` a slug, `restore`'s `rev` a commit — and a field that fails its type, or one the tool does not declare, comes back as a tool error with nothing run. A call that passes execs `pair.sh` as an argv list with no shell between them, from the checkout `CLAUDE_PROJECT_DIR` names, and returns a JSON object carrying `exit`, `stdout` and `stderr` verbatim, so the stdout contract above reaches the caller unchanged. The server exists because every `Bash` call is wrapped: `hooks/bwrap-wrap.py` binds every lane directory read-only in every checkout for every caller, the main agent included, so a `pair.sh` typed into a shell meets its lanes read-only and `pair.sh` reaches the lane directories only through this server. No blind agent admits `mcp__plugin_gauntlet_pair__*` on its `tools:` line. `python3 scripts/mcp/pair_server.py --self-test` checks it.

A mechanical error in a test — a fixture typo, a bad import — that the `scrivener`'s own `test` call did not catch surfaces in the red run and goes back to it as the `juror`'s `INVALID`.
...
5. `${CLAUDE_PLUGIN_ROOT}/scripts/strike-diff.py`, run by the `pair` tool `mcp__plugin_gauntlet_pair__merge`, checks the landed diff against the block by name and by quoted assertion text. There is no red run and no post-merge reviewer round: none of the three shapes has an implementation phase, so the window those two watch does not exist.
...
Every reviewer also refuses a brief that leads it: a conclusion offered as settled fact, a ruling on scope, a question addressed to the reviewer, an alternative verdict, its own rules recited back. That refusal is `CONTEMPT: LEADING`, and it burns that agent — the leading is in its context now — so the bare brief goes to a fresh one.

The other refusal, `OBJECTION: EVASION`, runs the opposite way, and deliberately. A reviewer calls it on a re-review when the main agent's return neither did the named repair nor supplied the missing citation. Burning the reviewer there would reward the evasion: a fresh one holds none of the findings that were evaded, so the main agent would get a clean slate and could run the same evasion again. So an `EVASION` call leaves the reviewer open, holding its findings, and the main agent answers that call back to the same reviewer, by `SendMessage`, with the repair or the citation. An answer that evades again gets a second `OBJECTION: EVASION` from the same reviewer. Only contempt replaces a reviewer.
