...
The `gauntlet-arbiter`, the `gauntlet-testsmith` and the `gauntlet-juror` are the three that never read the implementation. Everything else in the repo exists to keep that true.
...
The certifier is also blind to the test it is judging in a second sense: it did not write it. The `gauntlet-testsmith` grading its own red run is the same conflict one stage down from an agent testing its own code, so the run output goes to a fresh agent that holds none of the reasons the test was written the way it was.
