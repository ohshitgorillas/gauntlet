...
- `gauntlet-bailiff`, a blind agent spawned once per merged block. It takes the five-line `TEST CHECK` brief `scripts/pair.sh merge` prints, reads the committed block, the tests that landed, and the test diff and red output that `merge` writes to `state/merge/<slug>.txt`, and returns `PIN`, `SOFT`, `MISSING` or `EXTRA` per behavior line, plus a row per rule 4, 6, 13 or 14 violation. A gate verdict of `NO EVIDENCE` names a merge-output path that is absent or zero bytes, and rules on nothing; the repair is to run `scripts/pair.sh merge <slug>` again and spawn a fresh bailiff. It writes nothing and holds no `Write`.
